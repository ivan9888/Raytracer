                    ...
                    for (Light light : lights) {
                        ...
                        if(closestIntersection.getObject().getLayers().isReflection()){
                            reflectColor=Raytracer.reflection(ray,closestIntersection,objects,
                                    new double[]{cameraZ + nearFarPlanes[0], cameraZ + nearFarPlanes[1]},true,reflectColor1,light.getPosition());
                        }
                        if(closestIntersection.getObject().getLayers().isRefraction())
                        {
                            reflectColor=Raytracer.refraction(ray,closestIntersection,objects,
                                    new double[]{cameraZ + nearFarPlanes[0], cameraZ + nearFarPlanes[1]},true,true,reflectColor1,light.getPosition());
                        ...
                        for (int colorIndex = 0; colorIndex < objColors.length; colorIndex++) {
                            ...
                            if(closestIntersection.getObject().getLayers().isReflection() || closestIntersection.getObject().getLayers().isRefraction() )
                            {
                                ...
                                if(shadowIntersection==null)// mover ya que puede que no le llegue luz pero a los reflejos si
                                {
                                    if(closestIntersection.getObject().getLayers().isReflection())
                                    {
                                        if (fallOff > 0)
                                    ...
                                    else
                                    {
                                        if (fallOff > 0) {
                                            ...
                                            objColors[colorIndex] = (reflectColor[colorIndex] * lightFallOff);
                                        } else {
                                            objColors[colorIndex] = (reflectColor[colorIndex] * lightNonFallOff);
                                        }
                                    }
                                }
                                else
